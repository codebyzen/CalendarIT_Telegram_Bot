import random
# from puzzle_image import puzzle_image

HORIZONTAL 	= 0
VERTICAL	= 1

class WordGrid:
	def __init__(self, size, words_file, cheated) -> None:
		self.width = size
		self.area = size**2
	...
	def make(self):
		...
		for col in range(self.area):
			self.grid.append(self.get_random_letter())
		...
		self.generate_with_words()


	# generating words from file
	def get_words(self):
		words = []
		file = open(self.words_file, 'r')
		...
		chosen = ''
		while chosen not in words or len(words)<num:
			chosen = random.choice(lines).strip().upper()
			words.append(chosen)
			if len(words) == len(lines) ** num:
				print(f'There are no other possible combination of {lines} into {num} str')
				chosen = ''
				break
		
		self.words = words

	def print_words_in_puzzle(self):
	...
	def can_generate(self):
		...
		for word in self.words:
			...
			if len(word) > self.width:
				print(f"\nYou entered a word ({word}) too big for the grid size! (Grid size: {self.width})")
				return False
		return True
	...
	def print(self):
		for y in range(self.width):
			for x in range(self.width):
				self.puzzle.append(self.grid[x + y * self.width])
			self.puzzle.append("\n")
	
	def get_random_letter(self):
		alphabet = 'йфяцычувскамепинртгоьшлбщдюзжхэъabcdefghijklmnopqrstuvwxyz'.upper()
		return alphabet[random.randint(0, len(alphabet)-1)]


# if __name__ == "__main__":
# 	grid = WordGrid(size=20, words_file='assets/words-ru.txt', cheated=True)
# 	au = puzzle_image()
# 	print(grid.words)
